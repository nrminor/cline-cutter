...
def unify_sample_names(metadata_df: pl.DataFrame, vcf_path: str) -> str:
    ...
    meta_samples = metadata_df.select("Sample ID").drop_nulls().to_series().to_list()
    ...
    with open(new_vcf, "w", encoding="utf-8") as vcf_handle:
        ...
        subprocess.run(
            ["gzip", "-c"], check=True, stdin=process.stdout, stdout=vcf_handle
        )
        process.wait()
...
def assign_clusters(metadata_df: pl.DataFrame, max_distance: int) -> pl.DataFrame:
    ...
    for i, coordinate in enumerate(coordinates):
        distmat[::, i] = [
            distance(coordinate, other_coord, ellipsoid="WGS-84").kilometers
    ...
    cluster_assignments = pl.DataFrame({"sample": sample_ids, "cluster": clusters})

    return cluster_assignments


def create_sample_lists(
    metadata_df: pl.DataFrame, max_distance: int, proportion: float, seed: int
    ...
    # uneven sampling
    clusters = (
        cluster_df.select("cluster").unique(subset="cluster").to_series().to_list()
    )
    uneven_down = np.random.choice(clusters, size=ceil(len(clusters) / 3))
    unchanged = cluster_df.filter(pl.col("cluster") != uneven_down[0])
    uneven_df = (
        cluster_df.filter(pl.col("cluster") == uneven_down[0])
        .sample(fraction=proportion, seed=seed)
        .vstack(unchanged)
        .select(pl.col("sample"))
    )
...
def main():
    ...
    metadata = (
        pl.read_excel(metadata_path, read_csv_options={"null_values": ["NA", ""]})
        .select(pl.col(["Sample ID", "Latitude", "Longitude"]))
        .drop_nulls()
    )

    # create sample lists with which to filter the VCF
    random_df, even_df, uneven_df = create_sample_lists(
        metadata, max_distance, proportion, seed
    )
