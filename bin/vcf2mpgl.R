...
mpgl_to_point <- compiler::cmpfun(function(genotype_likelihoods, mult_files){
  ...
  if(mult_files){
    total.mean.gl<-matrix(NA, nrow=nrow(genotype_likelihoods), ncol=ncol(genotype_likelihoods))
    ...
    a.mean.gl<-apply(genotype_likelihoods, c(1,2), function(x){temp<-sum(10^(-0.1*unlist(x)));sum((10^(-0.1*unlist(x)))*(0:max(ploidy)))/temp})
  }
...
#--------------------------------------------------------------------
vcf_to_mpgl <- compiler::cmpfun(function(vcffile, mult_files){
...
vcf_to_mpgl(vcffile, mult_files)
